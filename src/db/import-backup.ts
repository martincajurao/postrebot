...
const tx = db.transaction(() => {
  ...
  const insPkg = db.prepare(
    'INSERT INTO packages (name, description, photo_url, base_price, discount, selections, active, is_fixed, is_custom) VALUES (?, ?, ?, ?, ?, ?, 1, 1, 0)'
  );
...
try {
  ...
  for (const p of db.prepare('SELECT id, name, base_price, discount, selections FROM packages ORDER BY id').all() as any[]) {
