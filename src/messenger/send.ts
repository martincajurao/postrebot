...
async function imageUrlOk(url: string): Promise<boolean> {
  // Trust Supabase Storage public URLs — they are generated by the SDK and
  // guaranteed to be publicly accessible (the bucket is public by config).
  // The pre-flight GET from the server often fails due to network/DNS/firewall
  // differences between the server and browser contexts, which would wrongly
  // drop valid images from carousels.
  try {
    const u = new URL(url);
    if (u.hostname.includes('supabase') && u.pathname.includes('/storage/v1/object/')) {
      return true;
    }
  } catch { /* not a URL, fall through to fetch check */ }
  try {
...
export async function sendCateringMenu(psid: string): Promise<void> {
  ...
  try {
    ...
    console.log(`[sendCateringMenu] total images in bucket: ${images.length}`);
    const tagged = images.filter((img) => String((img as any).name || '').startsWith('catering-'));
    console.log(`[sendCateringMenu] catering-tagged images: ${tagged.length}`);
    tagged.forEach((img) => console.log(`  - ${(img as any).name} → ${img.url}`));
    cateringImages = tagged
