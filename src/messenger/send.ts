...
async function imageUrlOk(url: string): Promise<boolean> {
  // Trust ALL Supabase Storage URLs — the path pattern is unique to Supabase
  // regardless of whether the hostname is *.supabase.co or a custom domain.
  // The pre-flight GET from the server often fails due to network/DNS/firewall
  // differences between server and browser contexts, which would wrongly drop
  // valid images from carousels.
  try {
    const u = new URL(url);
    if (u.pathname.includes('/storage/v1/object/')) {
      console.log(`[imageUrlOk] trusting Supabase URL: ${url}`);
      return true;
    }
