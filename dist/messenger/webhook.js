...
exports.setRequestOrigin = setRequestOrigin;
...
function encodeUrlPath(url) {
    ...
    const encoded = rawPath.split('/').map((seg) => {
        ...
        catch { /* raw '%' in name — keep and escape below */ }
...
function absUrl(url) {
    ...
    if (!base)
        return undefined; // no public URL known -> skip image
