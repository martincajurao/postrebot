...
r.put('/packages/:id', (req, res) => {
    ...
    active = COALESCE(?, active), is_fixed = COALESCE(?, is_fixed), is_custom = COALESCE(?, is_custom) WHERE id = ?`)
        .run(name ?? null, description ?? null, base_price ?? null, selections ?? null, active ?? null, is_fixed == null ? null : (is_fixed ? 1 : 0), is_custom == null ? null : (is_custom ? 1 : 0), req.params.id);
    res.json({ ok: true });
});
