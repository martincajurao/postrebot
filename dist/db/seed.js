...
const productCount = database_1.db.prepare('SELECT COUNT(*) c FROM products').get().c;
if (productCount > 0) {
    console.log('Seed skipped: products already exist.');
    process.exit(0);
}
function cat(name) {
    const row = database_1.db.prepare('SELECT id FROM categories WHERE name = ?').get(name);
    return row.id;
}
function product(category, name, description, m, l) {
    const out = database_1.db.prepare('INSERT INTO products (category_id, name, description) VALUES (?, ?, ?)')
...
// Delivery areas
const insArea = database_1.db.prepare('INSERT INTO delivery_areas (name, fee) VALUES (?, ?)');
insArea.run('Magarao', 50);
insArea.run('Naga City', 100);
insArea.run('Other Area', 150);
// Family Package — 4 selections
const fam = database_1.db.prepare('INSERT INTO packages (name, description, base_price, selections) VALUES (?, ?, ?, ?)')
...
database_1.db.prepare('UPDATE packages SET is_fixed = 1 WHERE id = ?').run(famId);
// Party Package — 5 selections
const party = database_1.db.prepare('INSERT INTO packages (name, description, base_price, selections) VALUES (?, ?, ?, ?)')
